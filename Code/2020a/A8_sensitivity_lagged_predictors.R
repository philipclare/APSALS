...
## SYNTAX FILE 8                                             ##
## SENSITIVITY ANALYSIS WITH LAGGED COVARIATES               ##
...
parallel::clusterEvalQ(cl, Lvars <- c("c_age1","hhavguse1","parmon1","homeacc1","alcrules1","alcmoney1","singlep1","smoking1","cbclextn1","cbcladn1","cbclwdn1","peeruse1","peerdis1","othsup1",
                                      "c_age2","hhavguse2","parmon2","homeacc2","alcrules2","alcmoney2","singlep2","smoking2","cbclextn2","cbcladn2","cbclwdn2","peeruse2","peerdis2","othsup2",
...
parallel::clusterEvalQ(cl, SLlib <- list(Q=c("SL.mean","SL.glm"),
                                         g=c("SL.mean","SL.glm")))
...
jtbinge6 <- parLapply(cl=cl, databinge6, function (x) {
  x <- x[,c(1:11,26,12:25,41,27:40,56,42:55,71,57:70,86,72:85,87)]
  ltmle(x,
...
jtresultss2 <- matrix(c(jtbingeres6,jtbingeres7,jtharmsres6,jtharmsres7,jtabuseres6,jtabuseres7,jtdependres6,jtdependres7,jtaudres6,jtaudres7),byrow=TRUE,ncol=4,nrow=5)
rownames(jtresultss2) <- c("Binge drinking","Any harms","DSM-IV Abuse","DSM-IV Dependence","DSM-5 AUD")
colnames(jtresultss2) <- c("Wave 6 log(OR)","Wave 6 SE","Wave 7 log(OR)","Wave 7 SE")

save(jtresultss2,file=paste0(cloudstor,"PhD/Paper 5 - APSALs application/Results/jtresults S2 20191028.RData"))
